...
def draw_image(image):
    ...
    n_rows, ncols = image.shape
    width, height = 1.0 / ncols, 1.0 / n_rows
...
def draw_policy(optimal_values):
    ...
    n_rows, n_cols = optimal_values.shape
        ...
        for i in range(WORLD_SIZE):
            for j in range(WORLD_SIZE):
                for action in ACTIONS:
                    ...
                    new_value[i, j] += ACTIONS_PROB * (reward + DISCOUNT * value[next_i, next_j])
    ...
    for i in range(WORLD_SIZE):
        for j in range(WORLD_SIZE):
            ...
            for a in ACTIONS:
                ...
                A[index_s, index_s_] += ACTIONS_PROB * DISCOUNT
                b[index_s] -= ACTIONS_PROB * r
