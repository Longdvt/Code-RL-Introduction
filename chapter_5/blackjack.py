...
# function from target policy of player
def target_policy_player(useable_ace_player, player_sum, dealer_card):
    return POLICY_PLAYER[player_sum]

# function from behavior policy of player
def behavior_policy_player(useable_ace_player, player_sum, dealer_card):
...
# get the value of a card (11 for case)
...
def play(policy_player, initial_state=None, initial_action=None):
    ...
    useable_ace_dealer = 1 in (dealer_card1, dealer_card2)
    ...
    while True:
        ...
        # if hit, get new card
        card = get_card()
        # keep track of the ace count. the usable_ace_player flag is insufficient alone as it cannot distinguish between having one ace or two
    ...
    while True:
        ...
        # if the dealer has a usable ace, use it as 1 to avoid busting anf continue
...
def monte_carlo_on_policy(episodes):
    ...
    # initialize counts to 1 to avoid 0 being devided
    ...
    for i in tqdm(range(0, episodes)):
        ...
        for (usabel_ace, player_sum, dealer_card), _ in player_trajectory:
            player_sum -= 12
            dealer_card -= 1
            if usabel_ace:
...
def monte_carlo_off_policy(episodes):
    ...
    weight_returns = rhos * returns

    weight_returns = np.add.accumulate(weight_returns)
    rhos = np.add.accumulate(rhos)

    ordinary_sampling = weight_returns / np.arange(1, episodes+1)
    with np.errstate(divide='ignore', invalid='ignore'):
        weight_sampling = np.where(rhos != 0, weight_returns / rhos, 0)
    return ordinary_sampling, weight_sampling
...
def figure_5_3():
    ...
    for i in tqdm(range(0, runs)):
        ordinary_sampling_, weight_sampling_ = monte_carlo_off_policy(episodes)
        # get the squared error
        error_ordinary += np.power(ordinary_sampling_ - true_value, 2)
        error_weighted += np.power(weight_sampling_ - true_value, 2)
    error_ordinary /= runs
    error_weighted /= runs
